...
import { Collections, ListPhoto, TotalApi, User } from '../types'
...
export default function SearchProvider({ children }: SearchProviderProps) {
    ...
    const handleSubmit = async (e?: React.ChangeEvent<HTMLFormElement>) => {
        e?.preventDefault()
        // api search photos
        const dataPhotos = await unsplashApi.searchPhotos({ query: value || keyword, page: 1 })
        setPhotos(dataPhotos.results)
        // api search collections
        const dataCollections = await unsplashApi.searchCollections({ query: value || keyword, page: 1 })
        ...
        // api search users
        const dataUser = await unsplashApi.searchUsers({ query: value || keyword, page: 1 })
